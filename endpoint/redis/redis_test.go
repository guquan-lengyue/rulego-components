...
func TestRedisEndpoint(t *testing.T) {
	...
	count := int32(0)
	// 路由1
	router1 := endpoint.NewRouter().From("device.msg.request,device.msg.response").Process(func(router endpointApi.Router, exchange *endpointApi.Exchange) bool {
	...
	router2 := endpoint.NewRouter().From("device.msg.request").Process(func(router endpointApi.Router, exchange *endpointApi.Exchange) bool {
		atomic.AddInt32(&count, 1)
		if exchange.In.Headers().Get("topic") == "device.msg.response" {
			assert.Equal(t, "this is response", exchange.In.GetMsg().Data)
			return false
